  ...
  const variants = {
    default: (
      <div className={`bg-base-100 p-6 rounded-lg shadow-xs ${className}`}>
        <div className="rating mb-4">{renderStars(rating)}</div>
        <blockquote className="text-base leading-relaxed mb-4">
          {highlight && <span className="bg-warning/25 px-1.5">{highlight}</span>} {text}
        </blockquote>
        {author.name && (
    ...
    small: (
      <div className={`space-y-6 max-w-lg mx-auto px-8 py-16 md:py-32 ${className}`}>
        <div className="rating flex! justify-center">{renderStars(rating)}</div>
    ...
    avatars: (
      <div
        className={`flex flex-col md:flex-row justify-center items-center md:items-start gap-3 ${className}`}
      >
        <div className="-space-x-5 avatar-group justify-start">
          {Array.isArray(author.avatars) &&
            author.avatars.map((avatar, i) => (
        ...
        <div className="flex flex-col justify-center items-center md:items-start gap-1">
          ...
          {text && <div className="text-base text-base-content/80">{text}</div>}
        </div>
      </div>
    ),
    card: (
      <figure
        className={`relative max-w-lg h-full p-6 md:p-10 bg-base-200 rounded-2xl max-md:text-sm flex flex-col ${className}`}
      >
        ...
        <figcaption className="relative flex items-center justify-start gap-4 pt-4 mt-4 md:gap-8 md:pt-8 md:mt-8 border-t border-base-content/5">
          <div className="w-full flex items-center justify-between gap-2">
            <div>
              <div className="font-medium text-base-content md:mb-0.5">{name}</div>
              {username && <div className="mt-0.5 text-sm text-base-content/80">@{username}</div>}
            </div>
    ...
    featured: (
      <div
        className={`relative max-w-lg h-full p-6 md:p-10 bg-base-200 rounded-2xl max-md:text-sm flex flex-col ${featured ? 'lg:col-span-2 lg:text-lg' : ''} ${className}`}
      >
        <blockquote className="relative flex-1">
          <p className="text-base-content/80 leading-relaxed">{text}</p>
        </blockquote>
  ...
  return (
    <section className={`py-24 px-8 max-w-7xl mx-auto ${className}`}>
      <div className="flex flex-col text-center w-full mb-20">
        <div className="mb-8">
          <h2 className="sm:text-5xl text-4xl font-extrabold text-base-content">{title}</h2>
        </div>
        {subtitle && (
