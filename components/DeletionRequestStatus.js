        ...
        <div className="flex-1">
          <h3
            className={`font-semibold mb-2 ${
              isVeryUrgent ? 'text-red-800' : isUrgent ? 'text-orange-800' : 'text-yellow-800'
            }`}
          >
          ...
          <div className="space-y-2 text-sm">
            <p
              className={`${
                isVeryUrgent ? 'text-red-700' : isUrgent ? 'text-orange-700' : 'text-yellow-700'
              }`}
            >
            ...
            <p
              className={`${
                isVeryUrgent ? 'text-red-600' : isUrgent ? 'text-orange-600' : 'text-yellow-600'
              }`}
            >
      ...
      {isVeryUrgent && (
        <div className="mt-3 p-2 bg-red-100 border border-red-300 rounded-sm">
          <p className="text-xs text-red-800 font-medium">
            🚨 Your account will be deleted very soon! If you want to keep your account, cancel the
            deletion request now.
          </p>
        </div>
      )}
    </div>
